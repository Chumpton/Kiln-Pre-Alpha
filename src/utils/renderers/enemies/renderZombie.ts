...
// This file contains the "Base" skeletal animation logic used by Zombie, Golem, Spitter, and Boss
// We duplicate it here so each can evolve independently if needed, or we can treat this as the "Zombie" renderer
...
export const renderZombie = (
    ...
    let animSpeed = 250;
    let scale = 1.0;
    let skinColor = '#e6e6e6'; // Bone White
    let shirtColor = '#a6a6a6';
    let pantsColor = '#737373';
    let headEmoji = '💀';
    ...
    const getArmState = (isFront: boolean) => {
        ...
        if (isFront && enemy.isAttacking) {
            ...
            if (progress < 0.2) {
                ...
                // Swing from windup (~-1.3 rad) to extended (~1.0 rad)
    ...
    const drawHand = (isFront: boolean) => {
        ...
        let handEmoji = '✋';
        if (enemy.isAttacking && isFront) handEmoji = '🥊';
