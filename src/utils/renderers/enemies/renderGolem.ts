...
export const renderGolem = (
    ...
    // --- CONFIG ---
    let animSpeed = 500; // Slower
    let scale = 1.4;
    const skinColor = '#525252'; // Dark Grey
    const shirtColor = '#404040';
    const pantsColor = '#262626';
    const headEmoji = '🗿';
    ...
    const drawHand = (isFront: boolean) => {
        ...
        ctx.filter = 'grayscale(1) brightness(0.5)'; // Dark Hand

        let handEmoji = '✋';
        if (enemy.isAttacking && isFront) handEmoji = '✊'; // Golem punch
    ...
    // Golem Legs (Thicker?) - Using standard for now just scaled
    ...
    // Golem Body Detail (Solid block)
    ctx.fillStyle = '#737373';
    ctx.fillRect(x - 4 * scale, shirtTopY + 10 * scale, 8 * scale, 20 * scale);
